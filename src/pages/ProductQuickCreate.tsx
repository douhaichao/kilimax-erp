...
import { ArrowLeft, Camera, Save, ArrowRight, CheckCircle, Scale, Plus, Trash2 } from 'lucide-react';
import { Category, UOM, Product } from '@/types/product';
...
const ProductQuickCreate = ({ categories = [], systemUOMs = [], onSave }: ProductQuickCreateProps) => {
  ...
  const handleSave = () => {
    if (onSave) {
      const product: Product = {
        ...
        cost: parseFloat(formData.price) * 0.7 || 0,
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-4">
          <Button variant="ghost" onClick={() => navigate('/product-management')}>
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Quick Create Product</h1>
            <p className="text-gray-600 mt-2">Create a new product with multi-UOM support</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center pt-6 border-t border-blue-200">
        ...
        <div className="flex space-x-2">
          <Button variant="outline" onClick={() => navigate('/product-management')} className="border-gray-300">
            Cancel
          </Button>
          {currentStep < 4 ? (
            <Button 
